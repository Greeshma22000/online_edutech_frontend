...
export default function Register({ onSuccess }) {
  ...
  return (
    <div className="min-h-[80vh] grid place-items-center bg-gradient-to-br from-brand-50 via-white to-accent-50 relative overflow-hidden">
      ...
      <div className="w-full max-w-xl rounded-3xl border border-amber-500 border-brand-100 bg-white p-9 shadow-sm">
        <div className="flex items-center gap-3">
          <span className="inline-flex h-12 w-12 items-center justify-center rounded-full text-white text-xl" style={{ backgroundImage: 'linear-gradient(135deg, #f25c54 0%, #f7b267 100%)' }}>✨</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-8">
          <div>
            ...
            <input className="mt-1.5 w-full border border-amber-500 rounded-xl p-3.5 text-lg focus:outline-none focus:ring-2 focus:ring-brand-500" type="text" placeholder="Jane Doe" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div>
            <label className="block text-base font-medium">Email</label>
            <input className="mt-1.5 w-full border border-amber-500 rounded-xl p-3.5 text-lg focus:outline-none focus:ring-2 focus:ring-brand-500" type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" />
          </div>
          <div>
            <label className="block text-base font-medium">Password</label>
            <input className="mt-1.5 w-full border border-amber-500 rounded-xl p-3.5 text-lg focus:outline-none focus:ring-2 focus:ring-brand-500" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="new-password" />
          </div>
          <div>
